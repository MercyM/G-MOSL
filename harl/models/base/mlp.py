...
class MLPBaseGraph(nn.Module):
    """A MLP base module."""

    def __init__(self, args, obs_shape):
        super(MLPBaseGraph, self).__init__()
...
class MLPBaseGraphCritic(nn.Module):
    """A MLP base module."""

    def __init__(self, args, obs_shape, num_agents, share_obs_shape):
        super(MLPBaseGraphCritic, self).__init__()
        ...
        obs_dim = obs_shape[0] * num_agents + share_obs_shape[0],
        # obs_dim = obs_shape[0] * num_agents

        if self.use_feature_normalization:
            self.feature_norm = nn.LayerNorm(obs_dim)

        self.mlp = MLPLayer(
            obs_dim[0], self.hidden_sizes, self.initialization_method, self.activation_func
            # obs_dim, self.hidden_sizes, self.initialization_method, self.activation_func
        )
