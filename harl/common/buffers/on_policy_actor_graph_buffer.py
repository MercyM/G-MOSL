...
class OnPolicyActorGraphBuffer:
    ...
    def __init__(self, args, obs_space, act_space, num_agents):
        ...
        if act_space.__class__.__name__ == "Discrete":
            ...
            self.father_actions = np.zeros(
                (self.episode_length, self.n_rollout_threads, num_agents * num_agents * act_space.n),
                dtype=np.float32)
        else:
            self.available_actions = None
            self.father_actions = np.zeros(
                (self.episode_length, self.n_rollout_threads, num_agents * num_agents * act_shape),
                dtype=np.float32)
    ...
    def insert(
        ...
        self.last_actions[self.step] = last_actions.copy()
    ...
    def after_update(self):
        ...
        self.father_actions[0] = self.father_actions[-1].copy()
    ...
    def feed_forward_generator_actor(
        ...
        last_actions = self.last_actions.reshape(-1, *self.last_actions.shape[2:])
        father_actions = self.father_actions.reshape(-1, *self.father_actions.shape[2:])
        if self.available_actions is not None:
        ...
        for indices in sampler:
            ...
            if self.factor is None:
                yield obs_batch, rnn_states_batch, actions_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, father_actions_batch, last_actions_batch
            else:
                factor_batch = factor[indices]
                yield obs_batch, rnn_states_batch, actions_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, factor_batch, father_actions_batch, last_actions_batch
    ...
    def naive_recurrent_generator_actor(self, advantages, actor_num_mini_batch):
        ...
        for start_ind in range(0, n_rollout_threads, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                last_actions_batch.append(self.last_actions[:, ind])
                father_actions_batch.append(self.father_actions[:, ind])
                if self.available_actions is not None:
            ...
            last_actions_batch = np.stack(last_actions_batch, 1)
            father_actions_batch = np.stack(father_actions_batch, 1)
            if self.available_actions is not None:
            ...
            last_actions_batch = _flatten(T, N, last_actions_batch)
            father_actions_batch = _flatten(T, N, father_actions_batch)
            if self.available_actions is not None:
    ...
    def recurrent_generator_actor(self, advantages, actor_num_mini_batch, data_chunk_length):
        ...
        if len(self.father_actions.shape) > 3:
            father_actions = self.father_actions[:-1].transpose(1, 0, 2, 3, 4).reshape(-1,
                                                                                       *self.father_actions.shape[2:])
        else:
            father_actions = _sa_cast(self.father_actions)
        if len(self.last_actions.shape) > 3:
            last_actions = self.last_actions[:-1].transpose(1, 0, 2, 3, 4).reshape(-1, *self.last_actions.shape[2:])
        else:
            last_actions = _sa_cast(self.last_actions)
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                father_actions_batch.append(father_actions[ind: ind + data_chunk_length])
                if self.available_actions is not None:
            ...
            father_actions_batch = np.stack(father_actions_batch, axis=1)
            if self.available_actions is not None:
            ...
            father_actions_batch = _flatten(L, N, father_actions_batch)
            if self.available_actions is not None:
