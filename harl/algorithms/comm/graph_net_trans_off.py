...
class GATEncoder(nn.Module):
    def __init__(self, args, n_xdims, gat_nhead, node_num, n_actions, num_agents):
        super(GATEncoder, self).__init__()
        self.node_num = node_num
        self.GAT1 = GATConv(n_xdims, n_actions * num_agents, heads=gat_nhead, concat=True, dropout=0.6)  # 0.3
        self.GAT2 = GATConv(n_actions * num_agents * gat_nhead, n_xdims, dropout=0.6)  # 0.3
        self.args = args
    ...
    def forward(self, x):
        self.cuda_transfer()
        edge_index = self.gen_edge_index_fc(self.node_num).cuda()
        if (len(x.shape) == 2):
            x = f.relu(self.GAT1(x.cuda(), edge_index))
            x = self.GAT2(x, edge_index)
        elif (len(x.shape) == 3):
            out_list = []
            for i in range(x.shape[0]):
                x_ = f.relu(self.GAT1(x[i].cuda(), edge_index))
...
class SingleLayerDecoder(nn.Module):
    def __init__(self, n_xdims, obs_shape, num_agents, decoder_hidden_dim, node_num, device=torch.device("cpu")):
        ...
        # self.tanh_ = f.tanh()
        self.fc_infors = nn.Linear(n_xdims * num_agents, obs_shape, bias=True)
        # self.fc_infors = nn.Linear(decoder_hidden_dim * num_agents, obs_shape, bias=True)
        self.init_weights()
        self.to(device)
    ...
    def forward(self, input, ep=1):
        ...
        # infors = torch.sigmoid(self.fc_infors((dot_l + dot_r).reshape(ep, -1)))
        # infors = torch.relu(self.fc_infors(input.reshape(ep, -1)))

        infors = torch.sigmoid(self.fc_infors(input.reshape(-1, self.n_xdims * self.num_agents)))
        ...
        return infors, self.samples, self.mask_scores, self.entropy, self.adj_prob
...
class Actor_graph(nn.Module):
    def __init__(self, args, obs_shape, n_actions, num_agents, device=torch.device("cpu")):
        ...
        self.n_xdims = obs_shape + num_agents
        ...
        self.decoder = SingleLayerDecoder(self.n_xdims, obs_shape, num_agents, self.decoder_hidden_dim, self.node_num,
                                          self.device)

        self.to(device)

    def forward(self, src, ep=1):
        encoder_output = self.encoder(src)  # 1.4.33
        infors, samples, mask_scores, entropy, adj_prob = self.decoder(encoder_output, ep=ep)
        ...
        ###########################################################
        log_softmax_logits_for_rewards = f.log_softmax(logits_for_rewards, dim=1)  ############# 以前沒有dim
        ...
        return infors, encoder_output, samples, mask_scores, entropy, adj_prob, log_softmax_logits_for_rewards, entropy_regularization
